...
const ppInferSegment = l => {
  ...
  if (s.includes("401k") || s.includes("tpa")) return "401k TPA";
  if (s.includes("cuso")) return "CUSO Partners";
  if (s.includes("universit")) return "Universities";
  if (s.includes("state")) return "State Programs";
  return "Banks & CUs";
};

// Email status badge helper — returns {icon, color} or null
const ppEmailBadge = l => {
  const s = (l.email_status || "").toLowerCase().trim();
  if (s === "bounced") return { icon: "✕", color: PP.red, label: "Bounced" };
  if (s === "replied") return { icon: "✓", color: PP.teal2, label: "Replied" };
  if (s === "emailed" || s === "sent") return { icon: "✉", color: PP.gold, label: "Emailed" };
  return null;
};
      ...
      <div style={{padding:22}}>
        {drawerTab==="overview"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 16px",fontSize:13,marginBottom:20,fontFamily:PPF}}>
            ...
            <div><span style={{color:PP.muted}}>Type: </span><span style={{color:PP.soft}}>{lead.institution_type||ppInferSegment(lead)}</span></div>
            <div><span style={{color:PP.muted}}>Assets: </span><span style={{color:PP.soft}}>{lead.asset_size||"—"}</span></div>
            {(()=>{const eb=ppEmailBadge(lead);return eb?<div style={{gridColumn:"1/3"}}><span style={{color:PP.muted}}>Email Status: </span><span style={{color:eb.color,fontWeight:700}}>{eb.icon} {eb.label}</span></div>:null;})()}
...
export default function Mary() {
  ...
  const [ppSegment, setPpSegment] = useState("All");
...
Keep each section short — 2 to 4 lines max. No long paragraphs. Use bullet points.`;
  ...
  const ppFiltered = useMemo(() => {
    ...
    if(ppFE==="not_emailed") r=r.filter(l=>!l.email_status||l.email_status.trim()==="");
    if(ppFE==="emailed") r=r.filter(l=>{ const s=(l.email_status||"").toLowerCase(); return s==="emailed"||s==="sent"; });
    if(ppFE==="email_replied") r=r.filter(l=>(l.email_status||"").toLowerCase()==="replied");
  ...
  const ppKpis = useMemo(() => ({
    total: ppSegLeads.length,
    due: ppSegLeads.filter(l=>ppIsOv(l.next_linkedin_followup_date)&&!["booked","second_call","not_interested","closed"].includes(l.status)).length,
    booked: ppSegLeads.filter(l=>l.status==="booked").length + ppSegLeads.filter(l=>l.status==="second_call").length,
    accepted: ppSegLeads.filter(l=>l.status==="accepted_dm").length,
  }), [ppSegLeads]);

  const ppEmailFunnel = useMemo(() => {
    const emailed = ppSegLeads.filter(l=>{ const s=(l.email_status||"").toLowerCase(); return s==="emailed"||s==="sent"; }).length;
    const replied = ppSegLeads.filter(l=>(l.email_status||"").toLowerCase()==="replied").length;
    const bounced = ppSegLeads.filter(l=>(l.email_status||"").toLowerCase()==="bounced").length;
    return { emailed, replied, bounced, rate: emailed ? Math.round((replied/emailed)*100) : 0 };
  }, [ppSegLeads]);
  ...
  const TABS = [
    { k: "today",    label: "Today" },
    { k: "inbox",    label: "Inbox",    badge: inboxEmails.length },
    { k: "chat",     label: "Chat" },
      ...
      {tab !== "pipeline" && tab !== "chat" && <header style={S.header}>
        <div style={S.headerRow}>
          ...
          <div style={{display:"flex",alignItems:"center"}}>
            {googleToken && <div style={{fontSize:10,color:"rgba(255,255,255,0.35)",cursor:"pointer"}} onClick={disconnectGoogle}>✓ Google connected</div>}
          </div>
        </div>
      ...
      <main style={tab === "pipeline" ? {...S.main, padding: 0, paddingBottom: "calc(52px + env(safe-area-inset-bottom, 0px))", overflowY: "hidden", display: "flex", flexDirection: "column", background: PP.navy} : tab === "chat" ? {...S.main, padding: 0, paddingBottom: "calc(60px + env(safe-area-inset-bottom, 0px))", overflowY: "hidden", display: "flex", flexDirection: "column"} : S.main}>
          ...
          <div style={S.anim}>
            ...
            {(tomorrowPreview || (new Date().getHours() >= 17 && !tomorrowPreview && !tomorrowLoading)) && (
              <div style={S.tomorrowCard}>
                ...
                {tomorrowLoading ? <div style={S.skelWrap}><div style={S.skel}/><div style={{...S.skel,width:"70%"}}/></div> : tomorrowPreview ? <div style={S.bText}>{tomorrowPreview}</div> : <div style={{fontSize:13,color:"rgba(255,255,255,0.35)",fontWeight:300}}>Tap "Load" to preview tomorrow's schedule</div>}
              </div>
            )}
                  ...
                  <div key={i} style={{...S.mini, justifyContent:"space-between"}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,flex:1}}>
                      ...
                      <div>
                        ...
                        {t.reason && <div style={{fontSize:11,color:"rgba(255,255,255,0.45)"}}>{t.reason}</div>}
                      </div>
                    </div>
                ...
                {replyingTo === i ? (
                  <div>
                    ...
                    <div style={{display:"flex",gap:8,marginTop:8}}>
                      ...
                      <button onClick={() => { setReplyingTo(null); setReplyText(""); }} style={{...S.gcBtn, background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.45)", border:"1px solid rgba(255,255,255,0.08)"}}>Cancel</button>
                    </div>
                  </div>
                ) : (
                  <div style={{display:"flex",gap:8}}>
                    <button onClick={() => { setReplyingTo(i); setReplyText(""); }} style={{...S.gcBtn, fontSize:11, padding:"5px 12px"}}>↩ Reply</button>
                    <button onClick={() => { setTab("chat"); setTimeout(() => { setInput(`Draft a reply to ${email.from?.replace(/<.*>/, "").trim()} about: "${email.subject}"`); inputRef.current?.focus(); }, 100); }} style={{...S.gcBtn, fontSize:11, padding:"5px 12px", background:"rgba(59,130,246,0.15)", color:"#60A5FA", border:"none"}}>✦ Ask Mary</button>
                    <button onClick={() => { persistDismissedEmailId(email.id); setInboxEmails(p => p.filter((_, idx) => idx !== i)); }} style={{...S.gcBtn, fontSize:11, padding:"5px 12px", background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.4)", border:"1px solid rgba(255,255,255,0.08)", marginLeft:"auto"}}>✕ Dismiss</button>
                  </div>
                )}
              </div>
            ))}
            ...
            <div style={S.chatHeader}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={S.chatAvatar}>
                  <img src="/finoveo-logo.png" alt="Finoveo" style={{width:22,height:22,objectFit:"contain"}} />
                </div>
                <div>
        ...
        {tab === "pipeline" && (
          <div style={{display:"flex",flexDirection:"column",height:"100%",background:PP.navy,color:PP.white,fontFamily:PPF,overflow:"hidden"}} onClick={()=>setPpSDD(null)}>
            ...
            {/* Segment chips */}
            <div style={{display:"flex",gap:5,padding:"8px 14px 4px",overflowX:"auto",flexShrink:0,scrollbarWidth:"none",msOverflowStyle:"none"}}>
              {PP_SEGMENTS.map(seg=>(
                <button key={seg} onClick={()=>{setPpSegment(seg);setPpView("all");setPpPgN(1)}} style={{padding:"5px 12px",fontSize:11,fontWeight:600,borderRadius:20,border:`1px solid ${ppSegment===seg?PP.teal:PP.border}`,background:ppSegment===seg?"rgba(0,219,168,.15)":"transparent",color:ppSegment===seg?PP.teal2:PP.muted,cursor:"pointer",fontFamily:PPF,whiteSpace:"nowrap",flexShrink:0,transition:"all .15s"}}>{seg}</button>
              ))}
            </div>

            {/* KPI cards */}
            <div style={{display:"flex",gap:8,padding:"6px 14px 6px",flexShrink:0}}>
              {[
              ...
              {!ppLoading && ppPaged.map(lead=>{
                ...
                return <div key={lead.id} onClick={()=>setPpSelectedLead(lead)} style={{background:PP.card,border:`1px solid ${due?"rgba(245,197,24,.35)":PP.border}`,borderRadius:12,padding:"12px 14px",marginBottom:8,cursor:"pointer",transition:"background .15s"}}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginTop:2}}>
                    ...
                    {(()=>{const eb=ppEmailBadge(lead);return eb?<span style={{fontSize:11,fontWeight:700,color:eb.color,fontFamily:PPF}}>{eb.icon} {eb.label}</span>:null;})()}
                  </div>
...
const S = {
  ...
  blob2: { display: "none" },
  blob3: { display: "none" },
  notifBanner: { background: "rgba(31,205,121,0.08)", borderBottom: "1px solid rgba(31,205,121,0.15)", padding: "10px 16px", position: "relative", zIndex: 10 },
  ...
  chatAvatar: { width: 38, height: 38, borderRadius: "50%", background: "rgba(0,180,170,0.08)", border: "2px solid rgba(0,180,170,0.7)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" },
